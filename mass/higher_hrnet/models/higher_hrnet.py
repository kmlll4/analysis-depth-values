...
class PoseHigherResolutionNet(nn.Module):
    ...
    def __init__(self, cfg, use_coord_conv=False, **kwargs):
        self.inplanes = 64
        self.tag_conv_op = CoordConv2d if use_coord_conv else nn.Conv2d
        ...
        self.num_seg_class = cfg.MODEL.NUM_SEG_CLASS
        self.dim_tag = cfg.MODEL.DIM_TAG
        self.loss_config = cfg.LOSS

        self.pretrained_layers = cfg['MODEL']['EXTRA']['PRETRAINED_LAYERS']

    def _make_final_layers(self, cfg, input_channels):
        dim_tag = cfg.MODEL.DIM_TAG
        num_joints = cfg.MODEL.NUM_JOINTS
        num_seg_class = cfg.MODEL.NUM_SEG_CLASS
        extra = cfg.MODEL.EXTRA
        ...
        final_layers.append(
            nn.ModuleDict({
                'hm_conv': nn.Conv2d(in_channels=input_channels,
                                     out_channels=num_joints,
                                     kernel_size=extra.FINAL_CONV_KERNEL,
                                     stride=1,
                                     padding=1 if extra.FINAL_CONV_KERNEL == 3 else 0),
                'tag_conv': self.tag_conv_op(in_channels=input_channels,
                                             out_channels=dim_tag,
                                             kernel_size=extra.FINAL_CONV_KERNEL,
                                             stride=1,
                                             padding=1 if extra.FINAL_CONV_KERNEL == 3 else 0),
                'seg_conv': nn.Conv2d(in_channels=input_channels,
                                      out_channels=num_seg_class,
                                      kernel_size=extra.FINAL_CONV_KERNEL,
                                      stride=1,
                                      padding=1 if extra.FINAL_CONV_KERNEL == 3 else 0)
            })
    ...
    def _make_deconv_layers(self, cfg, pre_stage_channels):
        num_joints = cfg.MODEL.NUM_JOINTS
        dim_tag = cfg.MODEL.DIM_TAG
        num_seg_class = cfg.MODEL.NUM_SEG_CLASS  # segmentation result is not concatenated
        extra = cfg.MODEL.EXTRA
        deconv_cfg = extra.DECONV
        ...
        for i in range(deconv_cfg.NUM_DECONVS):
            if deconv_cfg.CAT_OUTPUT[i]:
                pre_stage_channels += num_joints # + dim_tag  # + num_seg_class
                if deconv_cfg.CAT_TAG[i]:
                    pre_stage_channels += dim_tag
            ...
            pre_stage_channels = output_channels
    ...
    def forward(self, x):
        ...
        hm = self.final_layers[0]['hm_conv'](x)
        tag_output = self.final_layers[0]['tag_conv'](x)
        seg_output = self.final_layers[0]['seg_conv'](x)

        y = torch.cat([hm, tag_output], dim=1) if self.deconv_config.CAT_TAG[0] else hm
        # y = hm  # does not concat tag or segmentation
        ...
        for i in range(self.num_deconvs):
            ...
            hr_hm = self.final_layers[i + 1](x)

        return seg_output, hm, hr_hm, tag_output
...
class HigherHRNet(PoseHigherResolutionNet):
    """wrapper class to instantiate higherHRNet for COCO w32 size512"""
    def __init__(self,
                 weight=None,
                 num_keypoints=17,
                 num_seg_classes=91,
        ...
        LOSS = CN()
        LOSS.NUM_STAGES = 2
        LOSS.WITH_HEATMAPS_LOSS = (True, True)
        LOSS.WITH_AE_LOSS = (True, False)

        # general
        MODEL = CN()
        MODEL.NUM_JOINTS = num_keypoints
        MODEL.NUM_SEG_CLASS = num_seg_classes
        MODEL.DIM_TAG = dim_tag
        ...
        EXTRA.STAGE2.NUM_CHANNELS = [32, 64]
        ...
        EXTRA.STAGE3.NUM_CHANNELS = [32, 64, 128]
        ...
        EXTRA.STAGE4.NUM_CHANNELS = [32, 64, 128, 256]
        EXTRA.STAGE4.BLOCK = 'BASIC'
        EXTRA.STAGE4.FUSE_METHOD = 'SUM'

        EXTRA.DECONV = CN()
        EXTRA.DECONV.NUM_DECONVS = 1
        EXTRA.DECONV.NUM_CHANNELS = [32]
        EXTRA.DECONV.NUM_BASIC_BLOCKS = 4
        EXTRA.DECONV.KERNEL_SIZE = [4]
        EXTRA.DECONV.CAT_OUTPUT = [True]
        EXTRA.DECONV.CAT_TAG = [cat_tag]
        cfg = CN()
        cfg.MODEL = MODEL
        cfg.LOSS = LOSS
        cfg.MODEL.EXTRA = EXTRA
        super(HigherHRNet, self).__init__(cfg=cfg, **kwargs)

        if weight is not None:
            self.load_state_dict(torch.load(weight))
            print(f'weight {weight} loaded!')
