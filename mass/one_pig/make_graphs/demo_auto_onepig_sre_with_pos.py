...
class MassPredict:
    ...
    #     resnet18 = MassModelV3(
    #         backbone='resnet18', input_channels=args.input_channels,
    #         num_features=args.num_features)
    #     resnet34 = MassModelV3(
    #         backbone='resnet34', input_channels=args.input_channels,
    #         num_features=args.num_features)

    #     mobilenet = MassModelV3(
    #         backbone='mobilenet', input_channels=args.input_channels,
    #         num_features=args.num_features)
    #     xception = MassModelV3(
    #         backbone='xception', input_channels=args.input_channels,
    #         num_features=args.num_features)
    ...
    #     model_path = os.path.join(args.pretrained_weight)
    ...
    def load_mass_model(self, args):
        ...
        model = model = load_posture_model(args.model_name, args.input_channels, args.num_features, args.transfer, args.pretrained_weight)
    ...
    def pred_mass(self, bbox, ori_rgb, ori_mask, img, depth, mask, kp, floor_depth):
        ...
        return pred, pig, new_w, new_l, vis_img
...
def run(args, floor_depth, weight_mean, weight_std, data_dir, pig_id, target_weight, target_bbox, output_dir, bokeh_output_dir, scene_id, exp_id):
    ...
    csv_list = [["exp_id", "scene_id", "UID", "filename", "gt", "pred", "error", "edge_filter", "anomaly_wl", "anomaly_mask", "incomplete_mask", "all_judge", "w", "l", "img_idx", "img_name", "img_path"]]
    ...
    for i, filename in enumerate(tqdm(file_list)):
        ...
        pred, pig, pig_w, pig_l, vis_img = mass.pred_mass([x, y, w, h], ori_rgb, ori_mask, rgb_, depth_, mask_, kp_, floor_depth)
        ...
        if args.save_img:
            img_name = "{}_{}".format(pig_id, filename)
            img_path = os.path.join("imgs", str(exp_id), "{}_{}".format(pig_id, scene_id), img_name)
            os.makedirs(os.path.join(bokeh_output_dir, os.path.dirname(img_path)), exist_ok=True)
            # img_dest = os.path.join(output_dir, 'vis_img', '{:05d}.jpg'.format(i))
            img_dest = os.path.join(bokeh_output_dir, img_path)
            cv2.imwrite(img_dest, cv2.cvtColor(vis_img, cv2.COLOR_RGB2BGR))

        csv_onepig += [str(exp_id), str(scene_id), pig_id, filename, str(target_weight), str(pred), str(error), flag_edge_filter, flag_anomaly_wl, flag_anomaly_mask, flag_incomplete_mask, all_judge, pig_w, pig_l, str(process_number), img_name, img_path]
        csv_list.append(csv_onepig)

        process_number += 1

    # with open(os.path.join(output_dir, pig_id + '_sre_pos.csv'), 'w', newline='') as file_:
    with open(os.path.join(output_dir, pig_id + args.csv_name), "w", newline="") as file_:
...
if __name__ == "__main__":
    ...
    floor_depth_csv = "/workspace/mass/ecopork-main/one_pig/make_graphs/floor_depth_check.csv"
    ...
    # parser.add_argument('--pretrained_weight', type=str, default='/workspace/mass/ecopork-main/mass_models/2022110837_new_data/epoch-992_val_MRE-0.0327_val_narrow_MRE-0.0285.pth') # hrnet
    # parser.add_argument('--pretrained_weight', type=str, default='/workspace/mass/ecopork-main/mass_models/2022112501_hrnet_posturev100b/epoch-786_val_MRE-0.0541_val_narrow_MRE-0.0442.pth') # hrnet
    # parser.add_argument('--pretrained_weight', type=str, default='/workspace/mass/ecopork-main/mass_models/2022120102_hrnet_posturev110_transfer/epoch-339_val_MRE-0.0524_val_narrow_MRE-0.0424.pth') # hrnet
    # parser.add_argument('--pretrained_weight', type=str, default='/workspace/mass/ecopork-main/mass_models/epoch-1364_val_MRE-0.0453_val_narrow_MRE-0.0419.pth') # hrnet
    # parser.add_argument('--pretrained_weight', type=str, default='/workspace/mass/ecopork-main/mass_models/2022121347_new_data/epoch-288_val_MRE-0.0461_val_narrow_MRE-0.0436.pth') # ori
    ...
    floor_depth_df = pd.read_csv(floor_depth_csv)
    floor_depth_dict = dict(zip(floor_depth_df["bagfile_name"], floor_depth_df["from_visual"]))
    ...
    for target_dir in dir_list:
        ...
        floor_depth = floor_depth_dict[bagfile_name]
        ...
        #     print(bagfile_name)
        #     floor_depth = floor_depth_from_vis[bagfile_name] # pklから探索できないときは、目視確認から引っ張ってくる
        #     # continue
        # else:
        ...
        print("processing {}".format(pig_id))
        ...
        # continue
        run(args, floor_depth, weight_mean, weight_std, data_dir, pig_id, target_weight, target_bbox, output_dir, bokeh_output_dir, scene_id, exp_id)
