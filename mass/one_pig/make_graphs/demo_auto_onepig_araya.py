...
from rotater_wlgetter import RotaterInclKP
from anomaly_pig_filter import AnomalyPigFilter
from origin_filter import do_edge_filter
from utils import target_pig, shrink
from rotate_image import rotate_head_tail
...
class MassPredict:
    ...
    def pred_mass(self, bbox, img, depth, mask, kp):
        x, y, w, h = bbox
        d_mean = (depth * mask).mean()  # 間違っている方式
        ...
        height = np.where(depth != 0, self.floor_depth - depth.astype(int), 0)
        ...
        img, mask, height, kp, theta = ori_rotater(img, mask, height, kp)
        img, mask, height, kp = shrink(img, mask, height, kp)  # maskを利用して画像が丁度収まるようにshrinkさせる
        height *= mask
        # img, mask, depth = rotate_head_tail(img, mask, depth) # imao 試験追加 model_evaluationにはあり

        # sreフィルター用
        pig = {"shrinked_rgb": img.copy(), "shrinked_mask": mask.copy(), "shrinked_peak": kp.copy(), "W": w, "L": l}

        data = {"img": img, "depth": height, "mask": mask, "w": w, "l": l, "mm_squared": mm_squared}
        data = T.val_transforms(data)

        ## mask inputs 豚以外の背景を0にする
        mask = data["mask"]
        tensor = torch.cat([data["img"], data["depth"]], dim=0)
        tensor *= mask
...
def run(args, floor_depth, weight_mean, weight_std, data_dir, pig_id, target_weight, target_bbox, output_dir):
    ...
    csv_list = [["filename", "pred", "error", "edge_filter", "anomaly_mask", "incomplete_mask", "w", "l"]]
    ...
    for i, filename in enumerate(tqdm(file_list)):
        ...
        pred, pig, pig_w, pig_l = mass.pred_mass([x, y, w, h], rgb_, depth_, mask_, kp_)
        all_judge, _, flag_anomaly_mask, flag_incomplete_mask, _ = anomaly_pig_filter(pig)  # anomaly_wlは使用しない
        ...
        csv_onepig += [filename, str(pred), str(error), flag_edge_filter, flag_anomaly_mask, flag_incomplete_mask, pig_w, pig_l]
        csv_list.append(csv_onepig)

    with open(os.path.join(output_dir, pig_id + "_araya.csv"), "w", newline="") as file_:
...
if __name__ == "__main__":
    ...
    # parser.add_argument('--mass_model_path', type=str, default='/workspace/mass/ecopork-main/mass_models/epoch-926_val_MRE-0.0373_val_narrow_MRE-0.0314.pth')
    parser.add_argument("--mass_model_path", type=str, default="/workspace/origin_eval/ecopork-main/mass_models/2022120604_new_data/epoch-267_val_MRE-0.0472_val_narrow_MRE-0.0417.pth")
